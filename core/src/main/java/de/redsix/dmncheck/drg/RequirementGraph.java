...
public class RequirementGraph
    ...
    public static RequirementGraph from(final DmnModelInstance dmnModelInstance)
        ...
        final Collection<InputData> inputData =
            dmnModelInstance.getModelElementsByType(InputData.class);

        final RequirementGraph drg = new RequirementGraph(
            DefaultEdge.class,
        ...
        Stream.<Collection<? extends DrgElement>>of(
            decisions,
            knowledgeSources,
            inputData
        )
        ...
        for (KnowledgeSource knowledgeSource : knowledgeSources) {
            knowledgeSource
                ...
                .flatMap(RequirementGraph::collectDrgElements)
                .forEach(drgElement ->
                    drg.addEdge(drgElement, knowledgeSource)
                );
        }
    ...
    private static Stream<DrgElement> collectDrgElements(
