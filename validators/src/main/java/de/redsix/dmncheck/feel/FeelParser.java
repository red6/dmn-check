...
public final class FeelParser {
    ...
    private static final Parser<?> TOKENIZER = Parsers.or(
        ...
        Terminals.IntegerLiteral.TOKENIZER.or(
            Terminals.IntegerLiteral.TOKENIZER
        )
    );

    private static final Parser<FeelExpression> INTEGER =
        Terminals.IntegerLiteral.PARSER.map(Integer::valueOf).map(
            FeelExpression.IntegerLiteral::new
        );

    private static final Parser<FeelExpression> DOUBLE =
        Terminals.DecimalLiteral.PARSER.map(Double::valueOf).map(
            FeelExpression.DoubleLiteral::new
        );

    private static final Parser<FeelExpression> VARIABLE = Terminals.fragment(
        "variablefragment"
    ).map(FeelExpression.VariableLiteral::new);

    private static final Parser<FeelExpression> STRING = Terminals.fragment(
        "stringfragment"
    )
        .map(s -> s.substring(1, s.length() - 1))
        .map(FeelExpression.StringLiteral::new);

    private static final Parser<FeelExpression> BOOLEAN = Terminals.fragment(
        "booleanfragment"
    )
        .map(Boolean::valueOf)
        .map(FeelExpression.BooleanLiteral::new);

    private static final Parser<FeelExpression> NULL = Terminals.fragment(
        "nullfragment"
    ...
    private static Parser<FeelExpression> createRangeExpressionParser(
        final Parser<FeelExpression> expressionParser
    ) {
        return Parsers.or(
    ...
    private static Parser<FeelExpression> parseNot(
    ...
    private static Parser<FeelExpression.Empty> parseEmpty() {
        return Parsers.EOF.map(__ -> new FeelExpression.Empty()).or(
            Terminals.fragment("emptyfragment").map(__ ->
    ...
    private static Parser<FeelExpression> feelExpressionParser() {
        final Parser.Reference<FeelExpression> feelParserReference =
            Parser.newReference();

        final Parser<FeelExpression> literalParser = Parsers.or(
            INTEGER,
            DOUBLE,
            BOOLEAN,
            VARIABLE,
        ...
        final Parser<FeelExpression> parseRangeExpression =
            createRangeExpressionParser(literalParser);

        final Parser<
            FeelExpression
        > feelExpressionParserWithoutBinaryExpressions = Parsers.or(
            literalParser,
            NULL,
        ...
        final Parser<FeelExpression> feelExpressionParser =
            createBinaryExpressionParser(
                feelExpressionParserWithoutBinaryExpressions
            );
    ...
    private static boolean expressionLanguageIsFeel(
        ...
        try {
            ...
            return new Either.Left<>(
                ValidationResult.init.message(
                    "Could not parse '" + charSequence + "': " + e.getMessage()
                )
            );
        }
    }
    ...
    public static Either<
        ValidationResult.Builder.ElementStep,
        FeelExpression
    > parse(final Expression expression) {
