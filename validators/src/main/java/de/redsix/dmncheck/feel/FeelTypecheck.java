...
public final class FeelTypecheck {
    ...
    public static Either<ValidationResult.Builder.ElementStep, ExpressionType> typecheck(final Context context, final FeelExpression expression) {
    ...
    private static Either<ValidationResult.Builder.ElementStep, ExpressionType> typecheckDisjunctionExpression(final Context context, final FeelExpression head, final FeelExpression tail) {
    ...
    private static Either<ValidationResult.Builder.ElementStep, ExpressionType> typecheckBinaryExpression(final Context context, final FeelExpression left, final Operator operator, final FeelExpression right) {
    ...
    private static Either<ValidationResult.Builder.ElementStep, ExpressionType> checkOperatorCompatibility(final ExpressionType type, final Operator operator) {
        switch (operator) {
            case GE:
            case GT:
            case LE:
            ...
            case SUB:
                return check(ExpressionType.isNumeric(type),
                        "Operator " + operator + " expects numeric type but got " + type).orElse(right(type));
            case OR:
            case AND:
                return check(ExpressionTypes.BOOLEAN().equals(type),
                        "Operator " + operator + " expects boolean but got " + type).orElse(right(type));
            case NOT:
                return right(type);
            default:
    ...
    private static Either<ValidationResult.Builder.ElementStep, ExpressionType> typecheckRangeExpression(final Context context, final FeelExpression lowerBound, final FeelExpression upperBound) {
