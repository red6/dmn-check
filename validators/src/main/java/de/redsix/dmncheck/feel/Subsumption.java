...
final class Subsumption {
    ...
    private interface Comparison<A extends Comparable>
    ...
    private static final Comparison<?> ge = (a, b) ->
        a.compareTo(b) == 0 || a.compareTo(b) > 0;
    private static final Comparison<?> le = (a, b) ->
        a.compareTo(b) == 0 || a.compareTo(b) < 0;
    ...
    static Optional<Boolean> subsumes(
        ...
        return switch (expression) {
            ...
            case FeelExpression.BooleanLiteral(var aBool) -> switch (
                otherExpression
            ) {
                case FeelExpression.BooleanLiteral(
                    var anOtherBool
                ) -> Optional.of(comparison.test(aBool, anOtherBool));
                default -> Optional.of(false);
            };
            case FeelExpression.DateLiteral(var dateTime) -> switch (
                otherExpression
            ) {
                case FeelExpression.DateLiteral(
                    var anOtherDateTime
                ) -> Optional.of(comparison.test(dateTime, anOtherDateTime));
                default -> Optional.of(false);
            };
            case FeelExpression.DoubleLiteral(var aDouble) -> switch (
                otherExpression
            ) {
            ...
            case FeelExpression.IntegerLiteral(var anInteger) -> switch (
                otherExpression
            ) {
            ...
            case FeelExpression.StringLiteral(var aString) -> switch (
                otherExpression
            ) {
            ...
            case FeelExpression.VariableLiteral(
                var name
            ) -> subsumesVariableLiteral(name, otherExpression, comparison);
            case FeelExpression.RangeExpression(
                var leftInc,
                var lowerBound,
                var upperBound,
                var rightInc
            ) -> subsumesRangeExpression(
                leftInc,
                lowerBound,
                upperBound,
                rightInc,
    ...
    private static Optional<Boolean> subsumesVariableLiteral(
        ...
        return switch (otherExpression) {
            case FeelExpression.VariableLiteral(var otherName) -> Optional.of(
                comparison.test(name, otherName)
            );
            case FeelExpression.UnaryExpression(
                var operator,
                var operand
            ) -> Operator.NOT.equals(operator)
                ? subsumesVariableLiteral(name, operand, nq)
                : Optional.of(true);
    ...
    private static Optional<Boolean> subsumesUnaryExpression(
        ...
        return switch (otherExpression) {
            ...
            case FeelExpression.UnaryExpression(
                ...
                if (
                    operator.equals(otherOperator) &&
                    operand.equals(otherOperand)
                ) {
            ...
            default -> {
                if (
                    operator.equals(Operator.NOT) && otherExpression.isLiteral()
                ) {
    ...
    private static Optional<Boolean> subsumesRangeExpression(
        ...
        return switch (otherExpression) {
            case FeelExpression.RangeExpression(
                ...
                otherLowerBound,
                leftInc ? le : lt
            ).flatMap(subsumesLowerBound ->
                subsumes(
                    otherUpperBound,
                    upperBound,
                    rightInc ? le : lt
                ).flatMap(subsumesUpperBound ->
                ...
                if (otherExpression.isLiteral()) {
                    yield subsumes(
                        ...
                        subsumes(
                            ...
                            Optional.of(
                                subsumedByLowerBound && subsumedByUpperBound
                            )
                        )
                    );
                } else {
